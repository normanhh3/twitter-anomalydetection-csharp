...
namespace com.github.ruananswer.stl
{
	...
	public class STLUtility
	{
		...
		public static bool degreeCheck(int x)
		{
			return (x == 0 || x == 1 || x == 2);
		}
		...
		public static int getTWindow(int tDg, int sDg, int nS, int nP, double omega)
		{
			...
			double[][] coefsA = new double[][]
			{
				new double[] {0.000103350651767650, 3.81086166990428e-6},
				new double[] {-0.000216653946625270, 0.000708495976681902}
			};
			double[][] coefsB = new double[][]
			{
				new double[] {1.42686036792937, 2.24089552678906},
				new double[] {-3.1503819836694, -3.30435316073732},
				new double[] {5.07481807116087, 5.08099438760489}
			};
			double[][] coefsC = new double[][]
			{
				new double[] {1.66534145060448, 2.33114333880815},
				new double[] {-3.87719398039131, -1.8314816166323},
				new double[] {6.46952900183769, 1.85431548427732}
			};
		...
		public static double[] loessSTL(int[] x, double[] y, int span, int degree, int[] m, double[] weights, int jump)
		{
			...
			Array.Copy(@out.result, 0, res, 0, @out.result.Length);
		...
		private static double[] cInterp(int[] m, double[] fits, double[] slopes, double[] at)
		{
			...
			for (i = 0; i < n_at; ++i)
			{
				...
				u2 = u * u;
				u3 = u2 * u;
				ans[i] = (2 * u3 - 3 * u2 + 1) * fits[j] + (3 * u2 - 2 * u3) * fits[j + 1] + (u3 - 2 * u2 + u) * slopes[j] * h + (u3 - u2) * slopes[j + 1] * h;
			}
			return ans;
		}
